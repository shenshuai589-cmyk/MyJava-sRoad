...
> 如果表中没有主键，也没有唯一索引，InnoDB会自动生成一个rowId（DB_ROW_ID）作为隐藏的聚集索引
...
| ------ | ----------- | ------- | ---------- | ---- | ------------- | ------ | ------- | ------ | ------- | -------- | ------- |
| 查询的序列号 | 查询类型        | 正在访问的表名 | 匹配的分区      | 访问类型 | 可能用到的索引       | 实际用的索引 | 索引长度    | 索引比较的列 | 估计扫描的行数 | 过滤百分比    | 额外的重要信息 |
...
你想找一本“作者姓张，并且出版年份是 2020 年”的书。

图书馆里有一本《书名与作者目录册》**（这就是**二级索引）：

- 目录册上写着：`作者姓名`、`出版年份`、`书架号（主键ID）`。
    
- 真正的书都摆在远处巨大的书架上（这就是**主键索引 / 聚簇索引**）。
    

你的目标是去书架上**把真实的书拿出来**（这就是**回表查询**）。

### 1. 没有“索引下推”时（以前的做法）

你（Server 层）找管理员（存储引擎）：

> **你**：“帮我找‘姓张’且‘2020年出版’的书。”
> 
> **管理员**：翻开目录册，找到了 100 条“姓张”的记录。 管理员不管三七二十一，**跑去书架 100 次**，把这 100 本书全部抱到你面前（**发生了 100 次回表**）。
> 
> **你**：一本一本翻开封面看年份，发现只有 2 本是 2020 年的，另外 98 本全扔掉。

- **问题**：管理员白跑了 98 趟书架，累得半死，效率极低。
...
> **你**：“帮我找‘姓张’且‘2020年出版’的书。”
> 
> **管理员**：翻开目录册，找到了 100 条“姓张”的记录。 **但是这一次，管理员聪明了**！他直接看目录册上写着的出版年份，把不符合 2020 年的 98 条直接划掉（**在索引树上直接过滤**）。
> 
> 管理员发现只剩下 2 条记录满足条件，于是他**只跑了 2 趟书架**，把这 2 本书拿给你（**只发生了 2 次回表**）。

### 一句话总结

- **回表**：去书架拿真实书的过程（走磁盘 I/O，很慢）。
    
- **索引下推（ICP）**：在看目录册的时候，**能顺便过滤掉的条件就先过滤掉**，尽量**减少去书架拿书的次数**
